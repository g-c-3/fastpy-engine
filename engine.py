...
def main() -> int32:
    ...
    UCI loop runs in Python mode only (see __main__ block below).
...
# All code here runs only when the file is executed as Python.
#
# Usage: python engine.py   ← connect any UCI-compatible GUI
#
# Commands: uci, isready, ucinewgame, position startpos [moves ...],
...
if __name__ == '__main__':
    ...
    # ── Python-mode helpers: wrappers for compiled functions with uint64[218] ─
    #
    # Functions like alpha_beta(), generate_legal_moves(), and perft() declare
    # local `uint64[218]` arrays. FastPy emits these as C-style stack arrays.
    # In Python, bare `x: uint64[218]` leaves x unbound (annotation stored as
    # string, no value assigned). These wrappers replicate the logic using
    # Python lists instead.
    #
    # CRITICAL — Python reference semantics vs C++ value semantics:
    # make_move(board, move) modifies `board` in place (Python pass-by-reference).
    # In C++, the struct is copied automatically on function entry. In Python,
    # every call to make_move() must receive _copy.copy(board) so it operates
    # on a fresh copy, leaving the caller's board unchanged.
    # The compiled functions (alpha_beta, perft, etc.) are correct without copies
    # because C++ gives value semantics for free.

    def _generate_legal_moves_py(board):
        """Python-mode legal move generator (exact replica, Python list)."""
        pseudo = [0] * 218
        pcount = generate_all_moves(board, pseudo, 0)
        out = [0] * 218
        count = 0
        for i in range(pcount):
            nb = make_move(_copy.copy(board), pseudo[i])
    ...
    def _alpha_beta_py(board, depth, alpha, beta):
        ...
        for i in range(count):
            nb = make_move(_copy.copy(board), moves[i])
    ...
    def _find_best_move_py(board, depth):
        """Python-mode root search (exact replica, Python lists)."""
        moves, count = _generate_legal_moves_py(board)
        best_move  = 0
        best_score = NEG_INF
        alpha      = NEG_INF
        ...
        for i in range(count):
            m  = moves[i]
            nb = make_move(_copy.copy(board), m)
            score = -_alpha_beta_py(nb, depth - 1, -beta, -alpha)
            if score > best_score:
                best_score = score
                best_move  = m
    ...
    def _perft_py(board, depth):
        """Python-mode perft (exact replica, Python lists)."""
        moves, count = _generate_legal_moves_py(board)
        if depth == 1:
            return count
        nodes = 0
        for i in range(count):
            nodes += _perft_py(make_move(_copy.copy(board), moves[i]), depth - 1)
        return nodes
